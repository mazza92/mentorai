...
class PuppeteerCaptionFetcher {
  ...
  async getBrowser() {
    ...
    this.browserPromise = (async () => {
      ...
      console.log('[PuppeteerCaptionFetcher] ✓ Browser launched');
  ...
  async fetchCaptions(videoId) {
    ...
    try {
      ...
      if (captionData.error) {
        console.log(`[PuppeteerCaptionFetcher] ✗ ${captionData.error}`);
        return {
          success: false,
      ...
      console.log(`[PuppeteerCaptionFetcher] ✓ Success: ${segments.length} segments (${fullText.length} chars) in ${elapsed}ms`);
      ...
      const elapsed = Date.now() - startTime;
      console.error(`[PuppeteerCaptionFetcher] ✗ Error after ${elapsed}ms:`, error.message);
  ...
  async fetchMultiple(videoIds, concurrency = 5) {
    console.log(`[PuppeteerCaptionFetcher] 📦 Batch fetching ${videoIds.length} captions (concurrency: ${concurrency})`);
    ...
    console.log(`[PuppeteerCaptionFetcher] ✅ Complete: ${successful}/${videoIds.length} successful (${successRate}%)`);
